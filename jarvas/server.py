#!/usr/bin/env python3
"""
server.py — the JARVAS API and UI host.
...
class App:
    ...
    def boot(self) -> None:
        ...
        store.log_event("app", f"JARVAS {config.VERSION} started")
...
def node_hello(app: App, _m, _q, _b) -> dict:
    ...
    return {"app": "jarvas", "version": config.VERSION,
...
def setup_save(app: App, _m, _q, body) -> dict:
    ...
    # they asked, start JARVAS at login. Never fatal - a failed shortcut must
...
def catalog_all(app: App, _m, _q, _b) -> dict:
    """Every connector JARVAS offers, grouped, with live state for the natives."""
...
class JarvasHTTPServer(ThreadingHTTPServer):
...
def already_running(port: int, host: str = "127.0.0.1") -> bool:
    """True when a JARVAS instance is already serving this port."""
    from .connectors import base
    if not base.port_open(host, port, timeout=0.5):
        return False
    try:
        d = base.get(f"http://{host}:{port}/api/node/hello", timeout=2)
        return isinstance(d, dict) and d.get("app") == "jarvas"
...
def serve(app: App, port: int | None = None, bind: str | None = None,
    ...
    srv = JarvasHTTPServer((bind, port), Handler)
    srv.daemon_threads = True
    print(f"[jarvas] UI and API on http://{bind}:{port}", flush=True)

    if background:
        threading.Thread(target=srv.serve_forever, daemon=True,
                         name="jarvas-http").start()
        return srv
    try:
        srv.serve_forever()
