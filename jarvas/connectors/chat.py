#!/usr/bin/env python3
"""
chat.py — LLM provider abstraction for the JARVAS chat pane.
...
Also owns tool-calling: JARVAS can dispatch Hermes tasks, run sandbox
...
# Actions JARVAS may take from a chat turn. The model emits a fenced
# ```jarvas {...}``` block; we parse and execute it, then feed results back.
TOOL_SPEC = """
...
_FENCED_RE = re.compile(r"```(?:jarvas|json)?\s*(\{.*?\})\s*```", re.S)
