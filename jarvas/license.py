...
Two paths, both offline-tolerant:
  ...
  Offline key   A signed key the customer pastes in. Format:
                    JARVAS-<tier>-<expiry|0>-<seats>-<sig>
...
# Replace at build time (packaging/build_*.py reads JARVAS_SIGNING_SECRET).
SIGNING_SECRET = b"crosspcai-jarvas-default-secret"
...
def make_key(tier: str, expiry: int = 0, seats: int = 1) -> str:
    ...
    return f"JARVAS-{tier}-{expiry}-{seats}-{_sign(tier, expiry, seats)}"
...
def validate_key(key: str) -> dict:
    ...
    if len(parts) != 5 or parts[0] != "JARVAS":
        return {"ok": False, "error": "That does not look like a JARVAS key."}
