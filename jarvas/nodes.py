...
A JARVAS install on Ubuntu, Windows, a TrueNAS jail or a CrossPC AI OS box is
...
def add(name: str, url: str, token: str = "", role: str = "workstation") -> dict:
    ...
    if not probe.get("ok"):
        return {"ok": False, "error": f"no JARVAS node answered at {url}: "
...
def ping_url(url: str, token: str = "", timeout: float = 4.0) -> dict:
    ...
    # "jarvis" is the pre-rename identity. A fleet is upgraded one machine at a
    # time, so a node still on the old build must keep pairing rather than
    # looking like a stranger. Drop this once nothing reports the old name.
    if not isinstance(d, dict) or d.get("app") not in ("jarvas", "jarvis"):
        return {"ok": False, "error": "responded, but not a JARVAS node"}
    return {"ok": True, **d}
...
def discover(port: int = 5580, timeout: float = 0.35) -> list[dict]:
    """Scan the local /24 for other JARVAS installs. Runs only when the
