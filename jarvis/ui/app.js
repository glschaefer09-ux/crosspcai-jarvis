/* JARVIS — CrossPCAI control centre
...
async function openSession(sid) {
  ...
  if (!sid) {
    log.append(el('div', { class: 'empty' },
      el('div', { class: 'big' }, '◆'),
      el('div', {}, 'Ask JARVIS anything, or tell it what to do.'),
...
function editConnector(c) {
  modal({
    ...
    sub: 'Point JARVIS at any HTTP API. Your agents can then use it.',
    fields: [
...
async function loadNeeds() {
  ...
  if (!needs.length) {
    box.append(el('div', { class: 'empty' }, el('div', { class: 'big' }, '↑'),
      'Nothing outstanding. When JARVIS is missing something, it lands here.'));
    return;
  }
...
function pairNode() {
  modal({
    ...
    sub: 'Install JARVIS there, then point this one at it.',
    fields: [
...
async function scanNetwork() {
  ...
  if (!found.length) { toast('Nothing found', 'No other JARVIS installs answered.', 'err'); return; }
    ...
    grid.append(el('div', { class: 'card' },
      ...
      s.state === 'external'
        ? el('div', { class: 'meta', style: 'margin-top:5px' },
            'Something else already serves this port — JARVIS attached instead of starting its own.')
        : null,
...
async function loadSettings() {
  ...
  body.append(el('div', { class: 'card', style: 'margin-bottom:16px' },
    ...
    el('div', { class: 'meta', style: 'margin:6px 0 12px' },
      'When you ask for a connector or tool that does not exist, JARVIS can tell '
  ...
  auto.onchange = async () => {
    ...
    if (r.ok) toast(auto.checked ? 'JARVIS will start at login' : 'Start at login turned off', '', 'ok');
  ...
  body.append(el('div', { class: 'card', style: 'margin-bottom:16px' },
    ...
    el('div', { class: 'meta', style: 'margin:6px 0 12px' },
      ins.installed
        ...
        : 'JARVIS has not put an icon on this machine yet.'),
    el('label', { class: 'check' }, auto,
      el('span', {}, el('div', {}, 'Start JARVIS when I sign in'),
  ...
  body.append(section('Advanced', 'For pairing other machines and pointing at an existing stack.', [
    ...
    toast('Saved', 'Some changes need a restart of JARVIS.', 'ok');
  }));
}
...
function activateLicense() {
  modal({
    ...
    fields: [{ name: 'key', label: 'Licence key', placeholder: 'JARVIS-plus-0-3-…' }],
    okLabel: 'Activate',
    onOk: async (v) => {
...
const WIZ_STEPS = [
  {
    ...
    render: () => el('div', {},
      el('p', { class: 'meta', style: 'margin-bottom:14px' },
        'JARVIS is one app. It runs your agents, keeps a sandbox for work you do not '
      ...
      el('div', { class: 'field' },
        ...
        selectEl('wiz_role', 'workstation', [
          { value: 'workstation', label: 'My computer — I will use JARVIS here' },
  ...
  },
  {
    ...
    sub: 'JARVIS needs somewhere to think. Local costs nothing and stays on this machine.',
    render: () => el('div', {},
      el('div', { class: 'field' },
        el('label', {}, 'Provider'),
  ...
  },
  {
    ...
    render: () => {
      ...
      return el('div', {},
        el('p', { class: 'meta', style: 'margin-bottom:14px' },
          'When you ask JARVIS for a connector or tool it does not have, it makes a '
        ...
        el('div', { class: 'field', style: 'margin-top:14px' },
          ...
          inputEl('wiz_ingest', '', 'text', 'https://your-n8n/webhook/jarvis')));
    },
    collect: () => {
      WIZ.data.telemetry = {
...
async function wizardNext() {
  ...
  try {
    ...
    toast('JARVIS is ready', 'Your services are starting now.', 'ok');
    await boot(true);
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  boot().catch((e) => toast('JARVIS could not start', e.message, 'err'));
});
