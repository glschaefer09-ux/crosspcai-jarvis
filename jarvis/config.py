#!/usr/bin/env python3
"""
config.py — JARVIS configuration, paths and shared auth token.

Reuses the existing CrossPCAI conventions:
  ~/.crosspcai/agent_token   shared bearer token for hermes/sandbox/agent-api
  ~/.crosspcai/jarvis.json   JARVIS-specific settings
  ~/.crosspcai/jarvis.db     chat sessions + event log
"""
...
APP_NAME = "JARVIS"
APP_ID = "ai.crosspc.jarvis"
VERSION = "1.0.0"

CONFIG_DIR = Path(os.environ.get("CROSSPCAI_HOME", Path.home() / ".crosspcai"))
TOKEN_FILE = CONFIG_DIR / "agent_token"
CONFIG_FILE = CONFIG_DIR / "jarvis.json"
DB_FILE = CONFIG_DIR / "jarvis.db"
LOG_FILE = CONFIG_DIR / "jarvis.log"

# Ports of the existing CrossPCAI stack. JARVIS never re-implements these
# services — it is the single front-end that drives them.
DEFAULTS: dict = {
    "ui_port": int(os.environ.get("JARVIS_PORT", "5580")),
    ...
    "chat": {
        ...
        "system_prompt": (
            "You are JARVIS, the operations assistant for CrossPCAI. "
    ...
    "agents": {
        # Registry of named agents JARVIS can dispatch to via Hermes.
