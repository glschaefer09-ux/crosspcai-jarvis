#!/usr/bin/env python3
"""
supervisor.py — makes JARVIS a single self-contained application.
...
re-launches this same binary with `--service <id>`, so `jarvis.exe`,
`/usr/bin/jarvis` and the Docker image all contain the whole stack.
...
JARVIS attaches to it and reports "external" rather than starting a rival
...
def _launcher() -> list[str]:
    ...
    Frozen (PyInstaller): sys.executable IS jarvis.exe → ['jarvis.exe'].
    Source checkout:      ['python', '-m', 'jarvis'].
    """
...
class ManagedService:
    def __init__(self, sid: str, label: str, port: int, host: str = "127.0.0.1"):
        ...
        self.restarts = 0
        self.last_error = ""
        self.external = False  # port owned by a process JARVIS did not start
    ...
    def stop(self, timeout: float = 8.0) -> dict:
        if not self.running:
            return {"ok": True, "state": self.state(), "note": "not managed by JARVIS"}
        self.proc.terminate()
        try:
        ...
        def loop():
            while not self._stop.wait(interval):
                ...
                for svc in self.services.values():
                    # Only revive what JARVIS itself started; never fight an
                    # external systemd unit for the port.
                    if svc.proc is not None and not svc.running and not svc.port_live:
                        svc.restarts += 1
                        svc.start()

        self._watch = threading.Thread(target=loop, daemon=True, name="jarvis-watchdog")
        self._watch.start()
