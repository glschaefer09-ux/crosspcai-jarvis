#!/usr/bin/env python3
"""
__main__.py — the single entry point for every role JARVIS plays.

    jarvis                     desktop app: window + tray icon (the app icon)
    jarvis --server            headless: serves the same UI to a browser
    jarvis --service hermes    background daemon, launched by the supervisor
...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--install", action="store_true",
                   help="add the JARVIS icon and start it at login")
    p.add_argument("--no-autostart", action="store_true",
                   help="with --install: do not start JARVIS at login")
...
def run_installer(args) -> int:
    ...
    print(f"\n{config.APP_NAME} is installed."
          ...
          f"\n  Launch it from your applications menu, or run: jarvis")
    return 0 if res.get("ok") else 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    if already_running(port):
        ...
        print(f"[jarvis] already running — opening {url}")
        if not args.server:
            import webbrowser
            webbrowser.open(url)
        return 0
