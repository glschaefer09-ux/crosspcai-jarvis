...
and JARVIS keeps running with its agents alive, one click from the tray.
"""
...
def _start_tray(url: str, on_quit) -> "object | None":
    ...
    menu = pystray.Menu(
        pystray.MenuItem("Open JARVIS", lambda *_: webbrowser.open(url), default=True),
        pystray.MenuItem("Status", lambda *_: webbrowser.open(url + "/#/system")),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem("Quit", lambda icon, *_: (icon.stop(), on_quit())),
    )
    icon = pystray.Icon("jarvis", image, f"{config.APP_NAME} {config.VERSION}", menu)
    ...
    if not prefer_browser:
        ...
        if webview is not None:
            ...
            try:
                ...
                print(f"[jarvis] native window unavailable ({e}); using browser",
                      file=sys.stderr)
            else:
                if tray is None:
                    quit_app()
                    return 0
                print(f"[jarvis] window closed; still running in the tray - {url}")
                _idle(stopping)
                return 0
            del window

    # Browser fallback.
    print(f"[jarvis] open {url}")
    try:
        webbrowser.open(url)
    except Exception:
        pass
